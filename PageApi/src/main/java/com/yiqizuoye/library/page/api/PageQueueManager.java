...
class PageQueueManager {
     ...
     * 处理页面跳转
     */
    public synchronized static boolean processPageData(Activity context, PageQueue queue, RouterData routerData) {
     ...
     * 页面关闭时，调用该方法
     */
    public synchronized static void pageDataFinish(PageData pageData, Activity activity) {
        //不是队列界面 不做处理
        ...
        if (dataList == null || dataList.size() == 0) {
            ...
            new PageLauncher().start(activity, remove.routerData);
        }
    }
